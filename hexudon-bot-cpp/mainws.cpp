// ========================================================================
//  HEXUDON BOT v77.0-ws (WEBSOCKET TRANSPORT)
//  Phiên bản WebSocket — logic thuật toán 100% giống main.cpp (HTTP polling).
//  Chỉ khác hàm main(): dùng ws:// persistent connection thay vì GET/POST.
// ========================================================================
...
namespace cfg {
    ...
    constexpr int    TOP_CANDIDATES   = 50;
...
static string computeAssignment() {
    ...
    if (g_nAgents >= 5 && !g_spots.empty()) {
        ...
        if (g_nAgents >= 7 && (spotDispersion >= 6.0 || W >= 20 || workloadRatio > 1.2)) {
            nTankers = 2;
        } else if (g_nAgents >= 6 && (spotDispersion >= 8.0 || W >= 24)) {
            nTankers = 2;
        }
        ...
        for (;;) {
            ...
            for (int p = 0; p < nPatrols; p++) {
                ...
                for (auto& sp : g_spots) {
                    ...
                    // PASS 0: CHỈ LẤY BRAND CHƯA CÓ TRONG NGÀY HÔM NAY (Brand-First)
                    if (pass == 0 && teamPlannedBrands.count(sp.brand)) continue;
                ...
                for (const auto& cSpot : candidateSpots) {
                    ...
                    // Pass 1: Đi ăn thêm phần trùng brand thì phải giữ lại ít nhất 20% xăng cho ngày hôm sau
                    if (pass == 1 && teamPlannedBrands.count(spotBrand)) {
                        int minReserve = (int)(0.20 * g_maxFuel);
                        if (candSim.totalFuel > pFuel - minReserve) continue;
                    }
...
static void recordMatchBenchmark(const string& resultJson) {
    ...
    try {
        ...
        if (resObj) {
            // Hỗ trợ cả 2 format: {"standings":...} hoặc {"type":"result","data":{"standings":...}}
            const auto& standings = (*resObj)["standings"].isNull()
                ? (*resObj)["data"]["standings"]
                : (*resObj)["standings"];
            if (standings.size() > 0) {
...
// ========================================================================
// // ========================================================================
//  WEBSOCKET TRANSPORT — main()
//  Luồng:  connect ws → recv setup → send assignment
//        → loop { recv state → planActions → send actions }
//        → recv result → recordMatchBenchmark → done
//  Logic thuật toán (parseSetup, planActions, ...) 100% giống bản HTTP.
// ========================================================================
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (arg == "-transport" && i + 1 < argc)               i++; // bỏ qua, file này luôn dùng WS
        else if (arg[0] != '-') {
    ...
    // ── Chuẩn hóa URL WebSocket từ URL HTTP / WS ─────────────────────
    // Hỗ trợ cả các dạng:
    //   http://host:port             -> ws://host:port/ws/v1/matches/{id}?token={token}
    //   ws://host:port               -> ws://host:port/ws/v1/matches/{id}?token={token}
    //   http://host:port/ws/v1/...   -> không duplicate path
    string wsUrl;
    {
        string scheme = "ws://";
        string rest = url;
        if (rest.find("wss://") == 0) {
            scheme = "wss://";
            rest = rest.substr(6);
        } else if (rest.find("https://") == 0) {
            scheme = "wss://";
            rest = rest.substr(8);
        } else if (rest.find("ws://") == 0) {
            scheme = "ws://";
            rest = rest.substr(5);
        ...
        if (rest.find("/matches/") != string::npos || rest.find("/ws/") != string::npos) {
            wsUrl = scheme + rest;
            if (wsUrl.find("token=") == string::npos) {
                wsUrl += (wsUrl.find('?') == string::npos ? "?" : "&");
                wsUrl += "token=" + token;
            }
        } else {
            wsUrl = scheme + rest + "/ws/v1/matches/" + matchId + "?token=" + token;
        }
    }

    fprintf(stderr, "[WS] Connecting: %s\n", wsUrl.c_str());

    vector<string> wsHeaders;
    if (!token.empty()) {
        wsHeaders.push_back("Authorization: Bearer " + token);
    }

    ws::Client wsc;
    if (!wsc.connect(wsUrl, wsHeaders)) {
        fprintf(stderr, "[WS] FAILED — ket noi WebSocket that bai: %s\n", wsUrl.c_str());
        return 1;
    }

    // ── PHASE 1: Nhận SETUP từ server (server push) ──────────────────
    fprintf(stderr, "[WS] Waiting for setup...\n");
    string setupMsg;
    // Server có thể gửi message chào trước setup — chờ message chứa "map" hoặc "spots"
    for (int attempt = 0; attempt < 50; attempt++) {
        string msg = wsc.recv(10000); // 10s timeout cho mỗi message
        if (msg.empty()) {
            if (!wsc.ok()) {
                fprintf(stderr, "[WS] Mat ket noi trong luc cho setup!\n");
                return 1;
            }
            fprintf(stderr, "[WS] Dang cho setup (lan thu %d/50)...\n", attempt + 1);
            continue;
        }
        // Kiểm tra nhanh trước khi parse JSON đầy đủ
        if (msg.find("\"map\"") != string::npos || msg.find("\"spots\"") != string::npos ||
            msg.find("\"cells\"") != string::npos) {
            setupMsg = msg;
            break;
        }
        fprintf(stderr, "[WS] Skipping non-setup message (%zu bytes): %.80s...\n", msg.size(), msg.c_str());
    }
    if (setupMsg.empty()) {
        fprintf(stderr, "[WS] Khong nhan duoc setup sau 50 lan thu!\n");
        return 1;
    }
    fprintf(stderr, "[WS] Received setup (%zu bytes)\n", setupMsg.size());

    auto setupRoot = mj::parse(setupMsg);
    if (!setupRoot) {
        fprintf(stderr, "[WS] Parse setup JSON failed!\n");
        return 1;
    }
    // Auto-detect format: {"type":"setup","data":{...}} hoặc {"map":...,"spots":...}
    const mj::Value& setupData = (*setupRoot)["data"].isNull() ? *setupRoot : (*setupRoot)["data"];
    string assignBody = parseSetup(setupData);

    fprintf(stderr, "=== HEXUDON BOT v77.0-brand (BRAND-FIRST LEXICOGRAPHIC) ===\n");
    ...
    {
        // Server Go unmarshal trực tiếp vào []int
        if (!wsc.send(assignBody)) {
            fprintf(stderr, "[WS] Gui assignment that bai!\n");
            return 1;
        }
    }
    ...
    // ── PHASE 3: Vòng lặp chính — Nhận STATE, gửi ACTIONS ───────────
    int lastDay = -1;
    for (;;) {
        string msg = wsc.recv();
        if (msg.empty()) {
            if (!wsc.ok()) {
                fprintf(stderr, "[WS] Connection lost! Dang thu ket noi lai...\n");
                bool reconnected = false;
                for (int retry = 1; retry <= 5; retry++) {
                    sleepMs(400 * retry);
                    fprintf(stderr, "[WS] Thu reconnect lan %d/5...\n", retry);
                    if (wsc.connect(wsUrl, wsHeaders)) {
                        fprintf(stderr, "[WS] Reconnect thanh cong!\n");
                        reconnected = true;
                        break;
                    }
                }
                if (!reconnected) {
                    fprintf(stderr, "[WS] Reconnect that bai, ket thuc tran dau.\n");
                    break;
                }
                continue;
            }
        ...
        auto root = mj::parse(msg);
        if (!root) {
            fprintf(stderr, "[WS] Failed to parse message, skipping\n");
            continue;
        }

        // Detect message type (auto-detect cả 2 format JSON)
        string msgType;
        if (!(*root)["type"].isNull()) {
            msgType = (*root)["type"].str;
        }

        // Auto-unwrap {"type":"...","data":{...}} nếu có
        const mj::Value& data = (*root)["data"].isNull() ? *root : (*root)["data"];

        // ── RESULT ───────────────────────────────────────────────────
        if (msgType == "result" || !data["standings"].isNull() || !(*root)["standings"].isNull()) {
            fprintf(stderr, "[WS] Received RESULT\n");
            fprintf(stderr, "[RESULT] %s\n", msg.c_str());

            const char* envRunner = getenv("HEXUDON_BENCHMARK_RUNNER");
            if (!envRunner || string(envRunner) != "1") {
                recordMatchBenchmark(msg);
            }
            break;
        }

        // ── VALIDATION RESPONSE (Server xác nhận action hợp lệ) ─────
        if (!data["valid"].isNull()) {
            bool valid = data["valid"].boolean;
            int validDay = data["day"].asInt();
            if (!valid) {
                string reason = data["reason"].isNull() ? "" : data["reason"].str;
                fprintf(stderr, "[WS] [VALIDATION FAILED] Day %d khong hop le: %s\n", validDay, reason.c_str());
            } else {
                fprintf(stderr, "[WS] [VALIDATION OK] Day %d hop le!\n", validDay);
            }
            continue;
        }

        // ── STATE / DAY_STATE ────────────────────────────────────────
        bool isState = (msgType == "state" || msgType == "day_state" || msgType == "day" ||
                       (!data["day"].isNull() && !data["agents"].isNull()));
        if (isState) {
            int day = data["day"].asInt();
            if (day != lastDay) {
                auto dayStartTime = chrono::steady_clock::now();

                string acts = planActions(data);

                // Gửi actions dạng raw [][]int trực tiếp theo giao thức Server Go
                bool sent = wsc.send(acts);

                if (sent) {
                    auto elapsedMs = chrono::duration_cast<chrono::milliseconds>(
                        chrono::steady_clock::now() - dayStartTime).count();
                    fprintf(stderr, "[WS] Actions day %d da gui thanh cong (%lld ms)!\n", day, elapsedMs);
                    lastDay = day;
                } else {
                    fprintf(stderr, "[WS] FAILED to send actions for day %d!\n", day);
                }
            }
            continue;
        }

        // ── Unknown message — log chi tiết và bỏ qua ─────────────────────────
        fprintf(stderr, "[WS] Message (%zu bytes): %s\n",
                msg.size(), msg.c_str());
    }

    wsc.close();

    fprintf(stderr, "=== KET THUC TRAN DAU (WebSocket) ===\n");
